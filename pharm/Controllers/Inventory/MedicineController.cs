...
namespace pharm.Controllers.Inventory
{
    public class MedicineController : Controller
    {
        ...
        public ActionResult Index(/*string searchBy, string search*/)
        {
            ...
            ////{
            ////    return View(db.Medicine_Information.Where(x => x.Category1.Category1.Contains(search) || search == null).ToList());
            ////}
            ...
            var medicine_information = db.Medicine_Information.Include(m => m.Drug_Generic_name).Include(m => m.Manufacturer).Include(m => m.Unit).Include(m=>m.Category1);
        ...
        public ActionResult Create(Medicine_Information medicine_information)
        {
            ...
            ViewBag.CategoryId = new SelectList(db.Categories, "CategoryId", "Category1", medicine_information.CategoryId);
            ViewBag.UnitId = new SelectList(db.Units, "unitID", "unitName", medicine_information.UnitId);
            return View(medicine_information);
        }
        ...
        public ActionResult Edit(Medicine_Information medicine_information)
        {
            if (ModelState.IsValid)
            {
               ...
               // db.Entry(medicine_information).Property(r => r.Generic_ID).IsModified = true;
                db.Entry(medicine_information).Property(r => r.Manufacturer_ID).IsModified = true;
                db.Entry(medicine_information).Property(r => r.UnitId).IsModified = true;
                db.Entry(medicine_information).Property(r => r.CategoryId).IsModified = true;
